...
import { JobWizardFormValues, defaultJobWizardValues } from "@/app/(dashboard)/dashboard/jobs/_components/wizard/types"
import { createLocalizedJobWizardSchema } from "@/app/(dashboard)/dashboard/jobs/_components/wizard/validation"
import { Step1Basics } from "@/app/(dashboard)/dashboard/jobs/_components/wizard/step-1-basics"
import { Step2Criteria } from "@/app/(dashboard)/dashboard/jobs/_components/wizard/step-2-criteria"
import { Step3CandidateData } from "@/app/(dashboard)/dashboard/jobs/_components/wizard/step-3-candidate-data"
import { Step4ExamBuilder } from "@/app/(dashboard)/dashboard/jobs/_components/wizard/step-4-exam-builder"
import { Step5Review } from "@/app/(dashboard)/dashboard/jobs/_components/wizard/step-5-review"
...
// Helper function to map DB job data to wizard form format
function mapJobToFormValues(job: any): JobWizardFormValues {
    // Ensure screeningQuestions have idealAnswer field (for backward compatibility)
    const screeningQuestions = Array.isArray(job.screeningQuestions)
        ? job.screeningQuestions.map((q: any) => ({
            question: q.question || '',
            idealAnswer: q.idealAnswer !== undefined ? q.idealAnswer : true,
            disqualify: q.disqualify || false,
        }))
        : []
    ...
    return {
        ...
        employmentType: job.employmentType || 'remote',
...
export function EditJobPageClient({ jobId, userId }: EditJobPageClientProps) {
        ...
        const fetchJob = async () => {
            ...
            try {
                ...
                if (data.success && data.job) {
                    const formValues = mapJobToFormValues(data.job)
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.details && Object.keys(data.details).length > 0) {
                    ...
                    toast.error(data.error || t("jobWizard.validationError"))
                }
            }
        } catch (error) {
    ...
    if (jobNotFound) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen">
                ...
                <p className="text-muted-foreground mb-4">{t("jobs.jobNotFound")}</p>
    ...
    return (
        <div className="container mx-auto py-6 px-4 max-w-7xl">
            {/* Header with Stepper */}
            <div className="mb-6 pb-4 border-b bg-gradient-to-b from-background to-muted/20">
                <h1 className="text-xl font-semibold text-primary text-center mb-4">
                    {t("jobWizard.editJob")}
                </h1>
            ...
            {/* Content */}
            <div ref={contentRef} className="min-h-[60vh] overflow-y-auto">
                {/* Validation Errors */}
            ...
            {/* Footer */}
            <div className="mt-6 pt-4 border-t bg-muted/30 flex flex-col sm:flex-row items-center justify-between gap-4" dir={isRTL ? "rtl" : "ltr"}>
                {/* Previous Button */}
                <Button
                    type="button"
                    variant="outline"
